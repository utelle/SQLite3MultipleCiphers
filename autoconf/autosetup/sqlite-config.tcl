  ...
  set allFlags {
    ...
    packaging {
      ...
      {canonical autoconf} {
        ...
        soname:=legacy
          => {SONAME for libsqlite3mc.so. "none", or not using this flag, sets no
              soname. "legacy" sets it to its historical value of
              libsqlite3mc.so.0.  A value matching the glob "libsqlite3mc.*" sets
              it to that literal value. Any other value is assumed to be a
              suffix which gets applied to "libsqlite3mc.so.",
              e.g. --soname=9.10 equates to "libsqlite3mc.so.9.10".}
        # dll-basename: https://sqlite.org/forum/forumpost/828fdfe904
        dll-basename:=auto
          => {Specifies the base name of the resulting DLL file.
              If not provided, "libsqlite3mc" is usually assumed but on some platforms
...
proc sqlite-configure-phase1 {buildMode} {
  ...
  if {[file exists $srcdir/sqlite3mc.pc.in]} {
    proj-dot-ins-append $srcdir/sqlite3mc.pc.in
  }
...
########################################################################
# "soname" for libsqlite3mc.so. See discussion at:
...
proc sqlite-handle-soname {} {
  ...
  switch -exact -- $soname {
    none - "" { return 0 }
    legacy    { set soname libsqlite3mc.so.0 }
    default {
      if {[string match libsqlite3mc.* $soname]} {
        # use it as-is
      } else {
        # Assume it's a suffix
        set soname "libsqlite3mc.so.${soname}"
      }
    }
  }
...
proc sqlite-handle-mac-install-name {} {
  ...
  if {[proj-looks-like-mac]} {
    cc-with {-link 1} {
      if {[cc-check-flags "-Wl,-install_name,/usr/local/lib/libsqlite3mc.dylib"]} {
        define LDFLAGS_MAC_INSTALL_NAME {-Wl,-install_name,"$(install-dir.lib)/$(libsqlite3.DLL)"}
        set rc 1
      }
    }
...
# "libsqlite3mc", otherwise it may use a different value based on the
...
proc sqlite-handle-dll-basename {} {
  if {[proj-opt-was-provided dll-basename]} {
    ...
    if {$dn in {none default}} { set dn libsqlite3mc }
  } else {
    set dn libsqlite3mc
  }
  if {$dn in {auto ""}} {
    switch -glob -- [get-define host] {
      *-*-cygwin  { set dn cygsqlite3mc-0 }
      *-*-ming*   { set dn libsqlite3mc-0 }
      *-*-msys    { set dn msys-sqlite3mc-0 }
      default     { set dn libsqlite3mc }
    }
  }
...
# of libsqlite3mc.so.a files which are unnecessary in most environments.
#
...
#   /usr/lib/libsqlite3mc.dll.a despite the DLL being in
#   /usr/bin.
proc sqlite-handle-out-implib {} {
  ...
  if {[proj-opt-was-provided out-implib]} {
    ...
    if {$olBaseName in {auto ""}} {
      set olBaseName "libsqlite3mc" ;# [get-define SQLITE_DLL_BASENAME]
      # Based on discussions with mingw/msys users, the import lib
      # should always be called libsqlite3mc.dll.a even on platforms
      # which rename libsqlite3mc.dll to something else.
    }
